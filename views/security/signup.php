<?php
// Session + DB connection are bootstrapped via auth helper
    ...
    if ($userId === "" || $name === "" || $phone === "" || $email === "" || $dob === "" || $gender === "" || $pass === "" || $cpass === "") {
        ...
        try {
            ...
            if ($exists) {
                ...
                // Plain password (no hashing) per project requirement
            ...
            $error = $showDetails ? ('Server error: ' . $e->getMessage()) : 'Server error. Please try again.';
        }
    }
}
?>
